...
import { n as require__root, r as require_isObject, t as require__baseGetTag } from "./_baseGetTag-CP_ZWqH8.js";
//#region node_modules/balanced-match/index.js
var require_balanced_match$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/reduce-function-call/node_modules/balanced-match/index.js
...
//#endregion
//#region node_modules/reduce-function-call/index.js
var require_reduce_function_call = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	function getFunctionCalls(call, functionRE) {
		...
		do {
			...
			if (searchMatch[1] === void 0) throw new Error("Missing the first couple of parenthesis to get the function identifier in " + functionRE);
...
//#endregion
//#region node_modules/math-expression-evaluator/src/math_function.js
var require_math_function = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region node_modules/math-expression-evaluator/src/lexer.js
var require_lexer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	for (var i = 0; i < tokens.length; i++) tokens[i].precedence = preced[tokens[i].type];
	var type0 = {
		0: true,
		1: true,
		3: true,
	...
	function match(str1, str2, i, x) {
		for (var f = 0; f < x; f++) if (str1[i + f] !== str2[f]) return false;
		return true;
	}
	Mexp.tokenTypes = {
	...
	*/
	Mexp.addToken = function(newTokens) {
		for (var i = 0; i < newTokens.length; i++) {
			...
			var temp = -1;
			if (newTokens[i].type === Mexp.tokenTypes.FUNCTION_WITH_N_ARGS && newTokens[i].numberOfArguments === void 0) newTokens[i].numberOfArguments = 2;
			newAr[x] = newAr[x] || [];
			for (var y = 0; y < newAr[x].length; y++) if (newTokens[i].token === newAr[x][y]) {
			...
			if (temp === -1) {
				...
				if (newAr.length <= newTokens[i].token.length) newAr[newTokens[i].token.length] = [];
	...
	function indexOfToken(key, tokens) {
		for (var search = 0; search < tokens.length; search++) if (tokens[search].token === key) return search;
		return -1;
	}
	function tokenize(string) {
		...
		for (var i = 0; i < length; i++) {
			if (i < length - 1 && string[i] === " " && string[i + 1] === " ") continue;
			key = "";
			for (x = string.length - i > newAr.length - 2 ? newAr.length - 1 : string.length - i; x > 0; x--) {
				if (newAr[x] === void 0) continue;
				for (y = 0; y < newAr[x].length; y++) if (match(string, newAr[x][y], i, x)) {
					key = newAr[x][y];
					y = newAr[x].length;
					x = 0;
				}
	...
	Mexp.lex = function(inp, tokens) {
		...
		for (i = 0; i < nodes.length; i++) {
			...
			for (j = ptc.length; j--;) if (ptc[j] === 0) {
				if ([
					...
					if (allowed[cType] !== true) throw new Mexp.Exception(cToken + " is not allowed after " + prevKey);
		...
		for (j = ptc.length; j--;) str.push(closingParObj);
		if (allowed[5] !== true) throw new Mexp.Exception("complete the expression");
...
//#endregion
//#region node_modules/math-expression-evaluator/src/postfix.js
var require_postfix = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	Mexp.prototype.toPostfix = function() {
		...
		var arr = this.value;
		for (var i = 1; i < arr.length; i++) if (arr[i].type === 1 || arr[i].type === 3 || arr[i].type === 13) {
			if (arr[i].type === 1) arr[i].value = Number(arr[i].value);
			post.push(arr[i]);
		} else if (arr[i].type === 4) stack.push(arr[i]);
		else if (arr[i].type === 5) while ((popped = stack.pop()).type !== 4) post.push(popped);
		else if (arr[i].type === 11) {
			while ((popped = stack.pop()).type !== 4) post.push(popped);
			stack.push(popped);
		} else {
			elem = arr[i];
			pre = elem.pre;
...
//#endregion
//#region node_modules/math-expression-evaluator/src/postfix_evaluator.js
var require_postfix_evaluator = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	Mexp.prototype.postfixEval = function(UserDefined) {
		...
		for (var i = 0; i < arr.length; i++) if (arr[i].type === 1) stack.push({
			value: arr[i].value,
			type: 1
		});
		else if (arr[i].type === 3) stack.push({
			value: UserDefined[arr[i].value],
			type: 1
		});
		else if (arr[i].type === 0) if (typeof stack[stack.length - 1].type === "undefined") stack[stack.length - 1].value.push(arr[i]);
		else stack[stack.length - 1].value = arr[i].value(stack[stack.length - 1].value);
		else if (arr[i].type === 7) if (typeof stack[stack.length - 1].type === "undefined") stack[stack.length - 1].value.push(arr[i]);
		else stack[stack.length - 1].value = arr[i].value(stack[stack.length - 1].value);
		else if (arr[i].type === 8) {
			var popped = [];
			for (var x = 0; x < arr[i].numberOfArguments; x++) popped.push(stack.pop().value);
			stack.push({
				type: 1,
			...
			pop1 = stack.pop();
			if (typeof pop1.type !== "undefined") pop1 = [pop1];
			pop2 = stack.pop();
			pop3 = stack.pop();
			stack.push({
				type: 1,
		...
		} else if (arr[i].type === 13) if (bool) stack.push({
			value: UserDefined[arr[i].value],
			type: 3
		});
		else stack.push([arr[i]]);
		if (stack.length > 1) throw new Mexp.Exception("Uncaught Syntax error");
		return stack[0].value > 0x38d7ea4c68000 ? "Infinity" : parseFloat(stack[0].value.toFixed(15));
	};
	Mexp.eval = function(str, tokens, obj) {
		if (typeof tokens === "undefined") return this.lex(str).toPostfix().postfixEval();
		else if (typeof obj === "undefined") if (typeof tokens.length !== "undefined") return this.lex(str, tokens).toPostfix().postfixEval();
...
//#endregion
//#region node_modules/math-expression-evaluator/src/formula_evaluator.js
var require_formula_evaluator = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	Mexp.prototype.formulaEval = function() {
		...
		var disp = [];
		var arr = this.value;
		for (var i = 0; i < arr.length; i++) if (arr[i].type === 1 || arr[i].type === 3) disp.push({
			value: arr[i].type === 3 ? arr[i].show : arr[i].value,
			type: 1
		});
		else if (arr[i].type === 13) disp.push({
			value: arr[i].show,
			type: 1
		});
		else if (arr[i].type === 0) disp[disp.length - 1] = {
			value: arr[i].show + (arr[i].show != "-" ? "(" : "") + disp[disp.length - 1].value + (arr[i].show != "-" ? ")" : ""),
			type: 0
		};
		else if (arr[i].type === 7) disp[disp.length - 1] = {
...
//#endregion
//#region node_modules/reduce-css-calc/index.js
var require_reduce_css_calc = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**
	* Module dependencies
	*/
	var balanced = require_balanced_match$1();
	...
	function reduceCSSCalc(value, decimalPrecision) {
		...
		*/
		function evaluateExpression(expression, functionIdentifier, call) {
			...
			var unit = units[0] || "";
			if (unit === "%") expression = expression.replace(/\b[0-9\.]+%/g, function(percent) {
				return parseFloat(percent.slice(0, -1)) * .01;
			});
			...
			if (unit === "%") result *= 100;
			if (functionIdentifier.length || unit === "%") result = Math.round(result * decimalPrecision) / decimalPrecision;
			result += unit;
			return result;
		}
		/**
		...
		*/
		function evaluateNestedExpression(expression, call) {
			...
			while (matches = NESTED_CALC_RE.exec(nonEvaluatedPart)) {
				if (matches[0].index > 0) evaluatedPart += nonEvaluatedPart.substring(0, matches[0].index);
...
//#endregion
//#region node_modules/@visx/text/esm/util/getStringWidth.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_reduce_css_calc = /* @__PURE__ */ __toESM(require_reduce_css_calc());
...
//#endregion
//#region node_modules/@visx/text/esm/hooks/useText.js
var _excluded$1 = [
	"verticalAnchor",
	"scaleToFit",
	"angle",
...
function useText(props) {
	...
	var _useMemo = (0, import_react.useMemo)(function() {
		return {
			wordsWithWidth: (children == null ? [] : children.toString().split(/(?:(?!\u00A0+)\s+)/)).map(function(word) {
				return {
					word,
	...
	var wordsByLines = (0, import_react.useMemo)(function() {
		...
		if (width || scaleToFit) return wordsWithWidth.reduce(function(result, _ref) {
		...
		return [{ words: children == null ? [] : children.toString().split(/(?:(?!\u00A0+)\s+)/) }];
	}, [
		isXOrYNotValid,
		width,
		scaleToFit,
	...
	return {
		wordsByLines,
		startDy: (0, import_react.useMemo)(function() {
			return isXOrYNotValid ? "" : verticalAnchor === "start" ? (0, import_reduce_css_calc.default)("calc(" + capHeight + ")") : verticalAnchor === "middle" ? (0, import_reduce_css_calc.default)("calc(" + (wordsByLines.length - 1) / 2 + " * -" + lineHeight + " + (" + capHeight + " / 2))") : (0, import_reduce_css_calc.default)("calc(" + (wordsByLines.length - 1) + " * -" + lineHeight + ")");
		}, [
			isXOrYNotValid,
			verticalAnchor,
			capHeight,
		...
		transform: (0, import_react.useMemo)(function() {
			var transforms = [];
			if (isXOrYNotValid) return "";
...
//#endregion
//#region node_modules/@visx/text/esm/Text.js
var _excluded = [
	"dx",
	"dy",
	"textAnchor",
...
var SVG_STYLE = { overflow: "visible" };
function Text(props) {
	...
	props.capHeight;
	props.width;
	var textProps = _objectWithoutPropertiesLoose(props, _excluded);
	...
	return /* @__PURE__ */ import_react.createElement("svg", {
		...
		fontSize,
		style: SVG_STYLE
	}, wordsByLines.length > 0 ? /* @__PURE__ */ import_react.createElement("text", _extends({
		ref: innerTextRef,
		transform
	}, textProps, { textAnchor }), wordsByLines.map(function(line, index) {
...
//#endregion
export { Text, getStringWidth_default as getStringWidth, useText };
